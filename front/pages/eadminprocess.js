...
const contract = new ethers.Contract(address, ERC20_ABI,mysigner)
  ...
  {
     let mycin = document.querySelector('#identifier').value;
    console.log(mycin);
    if(document.querySelector('#verdict').value=="ok")
      {
        console.log("ok");
        await contract.ProcessCLient(true,mycin);}
    else
      {await contract.ProcessCLient(false,mycin);
      console.log("notok");}
  ...
  * {
  ...
  `}
            ...
            <h4>find client by id </h4>
                    <input  className="input-field" placeholder="national identity card number" type="text" name="identifier" id="identifier" />
                    <input  className="input-field" placeholder="verdict" type="text" name="verdict" id="verdict" />
                    <div className="action">
                    <Link href ='eadminfind'><button >find by id</button></Link>
                    <button onClick={e=>processclient()}>process by id</button>
                    </div>
                    <div id="app"></div>
                </div>
            </div>
